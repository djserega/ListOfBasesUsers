...
namespace ListOfBasesUsers
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private string _cultureDateTime = new CultureApp().GetFormatData();
        private bool _tableLoad = false;
        ...
        private void CbFilterPeriod_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            _tableLoad = false;
            // private string[] _arrayFilter = { "Сегодня", "Последняя неделя", "Последний месяц", "Текущий месяц" };
        ...
        private void DgTable_AutoGeneratedColumns(object sender, EventArgs e)
        {
            ...
            if (columnDateCreate != null)
                (columnDateCreate as DataGridTextColumn).Binding.StringFormat = _cultureDateTime;

            DataGridColumn columnDateEdit = dgTable.Columns.FirstOrDefault(f => (string)f.Header == "DateEdit");
            if (columnDateEdit != null)
                (columnDateEdit as DataGridTextColumn).Binding.StringFormat = _cultureDateTime;
        ...
        private void ChangeItemSoursedgTable()
        {
...
#if DEBUG
            Dialog.ShowMessage(datePEndEndDay.ToString());
#endif
            ...
            foreach (RowBase item in _listBases)
            {
                ...
                if (!(
                    item.DateCreate >= datePBegin.SelectedDate && item.DateCreate <= datePEndEndDay
